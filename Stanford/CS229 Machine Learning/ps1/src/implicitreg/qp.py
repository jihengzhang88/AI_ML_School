...
class QP:
    ...
    def gradient(self, X, Y):
        ...
        grad_theta = (errors[:, np.newaxis] * X * self.theta).mean(axis=0)
        grad_phi = - (errors[:, np.newaxis] * X * self.phi).mean(axis=0)
...
def QP_model_initialization(train_path, valid_path):
    ...
    for i in range(len(alphas)):
        ...
        print("initialization: {:.2f}".format(alpha), "final validation error: ", model.validation(X_val, Y_val))
