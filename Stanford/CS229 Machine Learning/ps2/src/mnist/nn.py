...
def forward_prop(data, one_hot_labels, params):
    ...
    loss = -np.sum(one_hot_labels * np.log(a2 + 1e-9)) / data.shape[0]
...
def backward_prop(data, one_hot_labels, params, forward_prop_func):
    ...
    a1, a2, loss = forward_prop_func(data, one_hot_labels, params)
    m = data.shape[0]
...
def gradient_descent_epoch(train_data, one_hot_train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    for i in range(0, m, batch_size):
        ...
        data_batch = train_data[i:end]
        labels_batch = one_hot_train_labels[i:end]

        grads = backward_prop_func(data_batch, labels_batch, params, forward_prop_func)
